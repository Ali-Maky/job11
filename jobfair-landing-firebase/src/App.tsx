import React, { useMemo, useState, FormEvent, ChangeEvent } from "react";
...
// Type for the "Add Vacancy" modal form (allows strings)
type NewJobInput = {
  id?: string;
  title?: string;
  company?: string;
...
// --- Helpers ---
const classNames = (...xs: (string | boolean | undefined)[]) => xs.filter(Boolean).join(" ");
...
function JobModal({ job, onClose, onSubmitted }: JobModalProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        exit={{ y: 40, opacity: 0 }}
        className="relative z-10 w-full max-w-3xl bg-white shadow-xl md:rounded-3xl h-[90svh] md:h-auto max-h-[90svh] md:max-h-[85vh] overflow-hidden flex flex-col"
        role="dialog" aria-modal="true"
      >
        {/* Header */}
